...
for prune_iteration in range(8):
    ...
    # Identify least signficant attention head
    ...
    for l in range(len(model.transformer_blocks)):
        for i in range(model.transformer_blocks[l].attn.n_heads):
            ...
            head_mask = model.transformer_blocks[l].attn.head_mask

            if head_mask[i]: continue
            ...
            if loss < min_loss:
                min_loss = loss
                layer, head_index = l, i

    # Prune and reinitialize

    model.transformer_blocks[layer].attn.head_mask[head_index] = False
