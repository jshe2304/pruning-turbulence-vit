...
from .utils.io_utils import get_npy_files, get_mat_files_in_range, frame_generator
...
def perform_long_analysis(
    ...
    PDF_U: bool,
    PDF_Omega: bool,
    long_analysis_emulator: bool, 
    long_analysis_train: bool, 
    long_analysis_truth: bool, 
    truth_frame_range: list[int], 
    spectra: bool, 
    analysis_length: int, 
    eof_ncomp: int, 
    ...
    """
    Perform long-run analysis on emulator, train, and truth datasets.
    ...
    Args
        ...
        long_analysis_emulator : Include emulator dataset in analysis loop if True.
        long_analysis_train : Include training dataset in analysis loop if True.
        long_analysis_truth : Include truth dataset in analysis loop if True.
    ...
    for dataset in ['train', 'truth', 'emulate']:
        ...
        if dataset == 'emulate':
            ...
            if long_analysis_train:
                # Load training data
                files = get_mat_files_in_range(os.path.join(data_dir,'data'), train_frame_range)
                print(f"Number of training .mat files: {len(files)}")
                output_dir_save = os.path.join(output_dir, 'train')
            else:
                continue

        elif dataset == 'truth':
            if long_analysis_truth:
                # Load training data
                # length of analysis for truth is same as emulator data
                files = get_mat_files_in_range(os.path.join(data_dir,'data'), truth_frame_range)
                print(f"Number of truth .mat files: {len(files)}")
                output_dir_save = os.path.join(output_dir, 'truth')
            else:
                continue
        ...
        frame_gen = frame_generator(
            dataset, files, inference_dir,
            data_dir, Kx, Ky, invKsq
        )
        ...
        for U, V, Omega in frame_gen:
            if dataset == 'emulate' and total_files_analyzed >= analysis_length:
        ...
        if temporal_mean:
            ...
            np.savez(os.path.join(output_dir_save, 'temporal_mean.npz'), U_sample_mean=U_mean, V_sample_mean=V_mean, Omega_sample_mean=Omega_mean, )
        ...
        if spectra:
            ...
            np.savez(os.path.join(output_dir_save, 'spectra.npz'), 
        ...
        if zonal_eof_pc or zonal_mean:
            ...
            np.savez(os.path.join(output_dir_save, 'zonal_mean.npz'), U_zonal_mean=U_zonal_mean, Omega_zonal_mean=Omega_zonal_mean, V_zonal_mean=V_zonal_mean, )
            ...
            if zonal_eof_pc:
                ...
                if dataset == 'train' or dataset == 'truth':
                    n_lags = target_step * PC_autocorr_nlags
                else:
                ...
                # np.savez(os.path.join(output_dir_save, 'zonal_eof_pc.npz'), EOF_U=EOF_U, PC_U=PC_U, exp_var_U=exp_var_U, EOF_Omega=EOF_Omega, PC_Omega=PC_Omega, exp_var_Omega=exp_var_Omega, EOF_U_sklearn=EOF_U_sklearn, PC_U_sklearn=PC_U_sklearn, expvar_U_sklearn=expvar_U_sklearn, EOF_Omega_sklearn=EOF_Omega_sklearn, PC_Omega_sklearn=PC_Omega_sklearn, expvar_Omega_sklearn=expvar_Omega_sklearn, EOF_U_svd=EOF_U_svd, PC_U_svd=PC_U_svd, expvar_U_svd=expvar_U_svd, EOF_Omega_svd=EOF_Omega_svd, PC_Omega_svd=PC_Omega_svd, expvar_Omega_svd=expvar_Omega_svd)
                np.savez(os.path.join(output_dir_save, 'zonal_eof_pc.npz'), U_eofs=EOF_U, U_pc=PC_U, U_expvar=exp_var_U, U_pc_acf=PC_acf_U, Omega_eofs=EOF_Omega, Omega_PC=PC_Omega, Omega_expvar=exp_var_Omega, Omega_pc_acf=PC_acf_Omega)
        ...
        if return_period:
            np.savez(os.path.join(output_dir_save, 'extremes.npz'), U_max_arr=np.asarray(U_max), U_min_arr=np.asarray(U_min), V_max_arr=np.asarray(V_max), V_min_arr=np.asarray(V_min), Omega_max_arr=np.asarray(Omega_max), Omega_min_arr=np.asarray(Omega_min))

        if return_period_anomaly:
            np.savez(os.path.join(output_dir_save, 'extremes_anom.npz'), U_max_arr=np.asarray(U_max_anom_arr), U_min_arr=np.asarray(U_min_anom_arr), V_max_arr=np.asarray(V_max_anom_arr), V_min_arr=np.asarray(V_min_anom_arr), Omega_max_arr=np.asarray(Omega_max_anom_arr), Omega_min_arr=np.asarray(Omega_min_anom_arr), )

        if PDF_U:
            U_arr = np.array(U_arr)
            U_mean, U_std, U_pdf, U_bins, bw_scott = PDF_compute(U_arr)
            np.savez(os.path.join(output_dir_save, 'PDF_U.npz'), bw_scott=bw_scott, U_mean=U_mean, U_std=U_std, U_pdf=U_pdf, U_bins=U_bins)

        if PDF_Omega:
            Omega_arr = np.array(Omega_arr)
            Omega_mean, Omega_std, Omega_pdf, Omega_bins, bw_scott = PDF_compute(Omega_arr)
            np.savez(os.path.join(output_dir_save, 'PDF_Omega.npz'), Omega_mean=Omega_mean, Omega_std=Omega_std, Omega_pdf=Omega_pdf, Omega_bins=Omega_bins, bw_scott=bw_scott, U_mean=U_mean)
